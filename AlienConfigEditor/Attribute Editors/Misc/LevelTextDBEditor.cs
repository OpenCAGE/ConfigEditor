...
namespace AlienConfigEditor
{
    public partial class LevelTextDBEditor : DockContent
    {
        ...
        public LevelTextDBEditor()
        {
            ...
            string[] textPaths = Directory.GetFiles(SharedData.pathToAI + "/DATA/TEXT", "*.TXT", SearchOption.AllDirectories);
        ...
        private void ReadLevelTextDatabasesXML()
        {
            ...
            var xml = XDocument.Load(SharedData.pathToAI + "/DATA/level_text_databases.xml");
            ...
            foreach (XElement levelDB in levelDBs)
            {
                ...
                if (levelName != "globals" && !Directory.Exists(SharedData.pathToAI + "/DATA/ENV/PRODUCTION/" + levelName) && !Directory.Exists(SharedData.pathToAI + "/DATA/ENV/PRODUCTION/DLC/" + levelName))
                {
        ...
        private void ReadLocalTextDBs()
        {
            ...
            {
                string[] levels = Directory.GetDirectories(SharedData.pathToAI + "/DATA/ENV/PRODUCTION/");
            ...
            {
                string[] levels = Directory.GetDirectories(SharedData.pathToAI + "/DATA/ENV/PRODUCTION/DLC/");
                for (int i = 0; i < levels.Length; i++)
                {
                    string dirName = Path.GetFileName(levels[i]);
            ...
            for (int i = 0; i < levelFolders.Count; i++)
            {
                ...
                level.using_local_db = Directory.Exists(SharedData.pathToAI + "/DATA/ENV/PRODUCTION/" + levelFolders[i] + "/TEXT/");
                level.is_dlc_map = dlcMap;

                if (level.using_local_db)
                {
                    level.text_dbs = File.ReadAllLines(SharedData.pathToAI + "/DATA/ENV/PRODUCTION/" + levelFolders[i] + "/TEXT/text_db_list.txt").ToList();

                    string textPath = SharedData.pathToAI + "/DATA/ENV/PRODUCTION/" + (_levels[i].is_dlc_map ? "DLC/" : "") + _levels[i].name + "/TEXT";
        ...
        private void WriteLevelTextDatabasesXML()
        {
            ...
            File.WriteAllLines(SharedData.pathToAI + "/DATA/level_text_databases.xml", output);
        }
